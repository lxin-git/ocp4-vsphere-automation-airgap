# Single Node Openshift deployment with Static IP

Start from openshift v4.9, RedHat announced the support for single node cluster deployment. OpenShift Container Platform on a single node is a specialized installation that requires the creation of a special ignition configuration ISO. The primary use case is for edge computing workloads, including intermittent connectivity, portable clouds, and 5G radio access networks (RAN) close to a base station. The major tradeoff with an installation on a single node is the lack of high availability.
After research, in addition to above, Single Node Openshift (SNO) can also be used by us in many other scenarios, and we can leverage SNO's benefits like:
...
- Quickly backup and restore a demo/poc environment, by building a single VM image, it's easily migrated to the customer environment to launch and start without product deployment & configuration.
...
- When the resources of a single node are sufficient enough, you can run cloud pak (non-production deployment). Currently, the deployment of aiops aimanager with xsmall size has been verified.

RedHat provided the steps [here](https://docs.openshift.com/container-platform/4.10/installing/installing_sno/install-sno-installing-sno.html) to provision SNO, however, this guide only support a DHCP enabled environment, the SNO bootstrap & master node has to obtain the IP address via DHCP client, while in real customer case, we won't be able allow deploy the cluster into a DHCP enabled environment. You have to use a manual configured static IP address for this all-in-one node. 

On the other hand, RedHat's tutorial suggest to use the Assisted Installer to perform the guided installation which is hard to automated and requiring the internet access. This is not an option for an air-gapped deployment or you have to create your own Assisted Installer service which is more complicated.

What we intend to do is make it simple without Assisted Installer and everything been automated.
...
1. Prepare the oc client binary, openshift-install binary, CoreOS live ISO 
file and SSH key to login to CoreOS. 
   ```sh
...
1. Create the install config file.  
...
1. Prepare coreos-installer command
   ...
   ```
   > Or you can download the **coreos-installer** binary from [here.](https://mirror.openshift.com/pub/openshift-v4/clients/coreos-installer)
...
1. Create a script which will be used to replace the bootstrap ignition content. 
   ...
   record_service_stage_success
   ```
   This could be a tricky one, SNO installation procedure will be: Launch the VM as the bootstrap node first with bootstrap ignition file, this ignition file include all the necessary actions to start bootstrap services which include generating the master ign file, then bootstrap node will write the master.ign file into master node disk(here is /dev/sda) through **install-to-disk** service, after that bootstrap node will reboot and become a master node, however, the **master.ign** won't include any network configuration, hence after reboot, the master node won't have ip address and require to obtain ip via DHCP. So we need embed the network configuration include ip/gateway settings to the **master.ign**. **coreos-installer install** command provide the option **-n** to copy existing network configuration to ign file. Which make us be able to pass the bootstrap network setting 
   to master node.
...
1. Encode the **install-to-disk** script and replace relevant content in bootstrap ign file.
   ```sh
   cp ocp/bootstrap-in-place-for-live-iso.ign iso.ign  # make a copy of original bootstrap ign file
   ``` 
   ```sh
...
1. Embed the bootstrap ign file to CoreOS live ISO file.
   ...
   ```
   > You can use **coreos-installer iso ignition show rhcos-live.x86_64.iso**
   > to check the resuslt.
...
1. Add static IP setting to ISO kernel arguments.
   ...
   ```
   Alternatively you can create an network connection file(eg. **sno-static-ip.nmconnection**) and use **coreos-installer iso network embed rhcos-live.x86_64.iso -k sno-static-ip.nmconnection** to embed the network setting to iso. Here I just use the **kargs modify** to embed it with kernel setting which is easier.
   The kernel argument consisted with : `"ip=${ip}::${gateway}:${netmask}:${hostname}:${interface}:none:${nameserver}"`

1. Now it's time to launch you SNO vm with the customized ISO file.

1. Post boot actions  
   After SNO vm boot via the customized ISO file, It will become a bootstrap node in the first time, no further action to take during this period, Once bootstrap process completed, the VM will automatic reboot and become master node. This time master node will have the static IP setting as we've already manipulated the master.ign file. but the hostname information won't be copied. Hence the master node will started with hostname **localhost**. We need correct it.

   If we don't want or don't have authority to add the DNS entry in public DNS server(in my example the public dns server is 9.9.9.9), we also need to configure the DNS resolve for SNO cluster. So after the 2nd boot, take the following steps to correct hostname 
   and DNS setting.
   ```shell
   ...
   nameserver 10.0.2.230
   nameserver 9.9.9.9
   ```

1. Wait for SNO ready  
